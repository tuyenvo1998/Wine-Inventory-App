...
export default function DetailScreen(props) {
  ...
  const updateBottle = (snap) => {
    ...
    if (!selectedBottle.image) return;
    else if (selectedBottle.image.startsWith("storage://"))
      firebase
        .storage()
        .ref()
        .child(selectedBottle.image.substr(10))
        .getDownloadURL()
        .then((url) => setBottleImage(url));
    else setBottleImage(selectedBottle.image);
  };
        ...
        <View style={styles.listContainer}>
          <ScrollView style={{ paddingTop: 22 }}>
            ...
            <Text style={styles.subtitle}>
              {bottle.type_of_wine} • {bottle.vintage} Vintage
            </Text>
            <Text style={styles.subtitle}>
            ...
            >
              <Icon.Button
                ...
                name={isFavorite() ? "heart-dislike-outline" : "heart"}
              >
            ...
            >
              {!isOpen() ? (
                ...
                <Text style={{ fontStyle: "italic", fontSize: 18 }}>
                  Opened on {dateString()}
                </Text>
              )}
            </View>
            <View style={{ padding: 16, fontSize: 16 }}>
              <Text style={styles.detail}>
                <Text style={{ fontWeight: "bold" }}>Varietal(s): </Text>
                {bottle.varietals}
              </Text>
              <Text style={styles.detail}>
                <Text style={{ fontWeight: "bold" }}>Age: </Text>
                {bottle.age} year{bottle.age !== 1 ? "s" : ""}
              </Text>
              <Text style={styles.detail}>
                <Text style={{ fontWeight: "bold" }}>Best served: </Text>
                {bottle.enjoy}
              </Text>
              <Text style={styles.detail}>
              ...
              <Text style={styles.detail}>
                <Text style={{ fontWeight: "bold" }}>Pairing(s): </Text>
                {(bottle.pairing || bottle.pairings).map((p, i, arr) => (
                  <Text
                    key={p}
                    style={{
                      textTransform: i === 0 ? "capitalize" : undefined,
                    }}
                  >
                    {p}
                    {i === arr.length - 1 ? "" : ", "}
